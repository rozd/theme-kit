...
///
/// Android-only workaround for SkipFuseUI's `ShapeStyle` lacking the `resolve(in:)` customization point.
///
...
nonisolated public struct AndroidResolvedStyle: Sendable {
    ...
    nonisolated public static let unsupported = AndroidResolvedStyle()

    nonisolated public static func style(_ shapeStyle: AnyShapeStyle) -> AndroidResolvedStyle {
        AndroidResolvedStyle(shapeStyle: shapeStyle)
    }

    nonisolated public static func shadow(_ shadow: AndroidShadow) -> AndroidResolvedStyle {
        AndroidResolvedStyle(shadow: shadow)
    }

    /// Composes a shadow onto this style, for `.surface.cardShadow` chaining.
    nonisolated public func adding(_ shadow: AndroidShadow?) -> AndroidResolvedStyle {
        AndroidResolvedStyle(shapeStyle: shapeStyle, shadow: shadow ?? self.shadow)
    }
}
...
nonisolated public protocol AndroidResolvableStyle {
    var androidResolvedStyle: AndroidResolvedStyle { get }
}

// MARK: - Conformances

extension Color: AndroidResolvableStyle {
    nonisolated public var androidResolvedStyle: AndroidResolvedStyle {
        .style(AnyShapeStyle(self))
    }
}

extension Gradient: AndroidResolvableStyle {
...
extension Shadow: AndroidResolvableStyle {
...
extension View {
    ...
    ///
    /// Android-only workaround for SkipFuseUI's missing shadow styling support.
    ///
    ...
    @ViewBuilder
    nonisolated public func androidThemeShadow(_ shadow: AndroidShadow?) -> some View {
        if let shadow {
            self.shadow(
