...
@Suite("AndroidViewModifiersGenerator")
struct AndroidViewModifiersGeneratorTests {
    ...
    @Test("Every wrapper view applies shadows via .androidThemeShadow")
    func allWrapperViewsApplyShadows() throws {
        ...
        // style must apply the shadow on every code path through .androidThemeShadow(rendering.shadow).
        // The test is written to automatically cover any new wrapper added later.

        let files = try ThemeFileGenerator().generate(from: try JSONDecoder().decode(ThemeFile.self, from: colorsOnlyJSON).styles, androidSupport: true)
        ...
        // For each wrapper view, verify it contains .androidThemeShadow( in its body
        for match in matches {
            if let range = Range(match.range, in: content) {
                ...
                // Only check if this is a wrapper view (has "body: some View" and calls androidThemeRendering)
                if structContent.contains("body: some View") && structContent.contains("androidThemeRendering") {
                    #expect(structContent.contains(".androidThemeShadow("),
                             "Wrapper view struct starting at \(structStart) must apply shadow via .androidThemeShadow()")
                }
            }
        }
    }
    ...
    func backgroundShapeView_shadowAppliesToShape() throws {
        ...
        #expect(structContent.contains(".androidThemeShadow(rendering.shadow)"),
                "Must apply androidThemeShadow to the filled shape")
        ...
        if let fillRange = structContent.range(of: "shape.fill(shapeStyle, style: fillStyle)"),
           let shadowRange = structContent.range(of: ".androidThemeShadow(rendering.shadow)") {
    ...
    func shadowColorNotInTemplate() throws {
        ...
        let files = try ThemeFileGenerator().generate(from: try JSONDecoder().decode(ThemeFile.self, from: fullWithShadowsJSON).styles, androidSupport: true)
        let viewModifiers = try #require(files.first { $0.name == "Android/View+AndroidThemeStyles.swift" })
    ...
    func generatedFile_hasAndroidGuard() throws {
        ...
        if let guardStart = content.range(of: "#if os(Android)"),
           let guardEnd = content.range(of: "#endif") {
            ...
            #expect(guardedContent.contains("protocol AndroidShapeStyleAdapter"),
    ...
    func wrappers_emittedAlways() throws {
        ...
        let viewModifiersNoShadows = try #require(
            files.first { $0.name == "Android/View+AndroidThemeStyles.swift" }
        )
        ...
        // Verify they all use .androidThemeShadow even when hasShadows=false
        #expect(viewModifiersNoShadows.content.contains(".androidThemeShadow(rendering.shadow)"),
                ".androidThemeShadow calls must be unconditional in all wrappers")
    }

    // MARK: - Wrapper body consistency

    @Test("ThemeStyledView uses androidThemeShadow")
    func themeStyledView_usesShadow() throws {
        ...
        #expect(structContent.contains("styled(rendering).androidThemeShadow(rendering.shadow)"),
                "ThemeStyledView body must call .androidThemeShadow on styled result")
    }

    @Test("ThemeStyledShapeView uses androidThemeShadow")
    func themeStyledShapeView_usesShadow() throws {
        ...
        let structContent = String(content[start.lowerBound...end])
        #expect(structContent.contains("filled(rendering).androidThemeShadow(rendering.shadow)"),
                "ThemeStyledShapeView body must call .androidThemeShadow on filled result")
    }
    ...
    func fileName() throws {
        let files = try ThemeFileGenerator().generate(from: try JSONDecoder().decode(ThemeFile.self, from: colorsOnlyJSON).styles, androidSupport: true)
        let viewModifiers = files.first { $0.name == "Android/View+AndroidThemeStyles.swift" }
