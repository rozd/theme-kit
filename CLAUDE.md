...
- `Color` encoding works on Android for colours built via `Color(hex:)`, which record their hex at construction (`Color+Hex.swift`); colours built from components still can't encode there.
